...
private:
    ...
    // Helper to get map value or fallback (declared here for internal use)
    uintptr_t GetOffsetOrFallback(const std::string& key, uintptr_t fallback) const;
...
public:
    ...
    // Offset for the UE AppendString function (used for FName conversion)
    uintptr_t GetAppendStringOffset() const;
};
